...
class UjianModel extends CI_Model
{
    ...
    {
        $this->load->database();
        $sql = "select F.* from (select E.* from(select D.*, un.nilai, un.created_at as tanggal from (select C.nama_mata_kuliah,C.nilai_minimum_kelulusan,C.id_ujian,C.kode_ujian,us.id as id_users, us.fullname, us.username from (select B.nama_mata_kuliah,B.nilai_minimum_kelulusan,B.kode_ujian,B.id_ujian,ku.id_users from 
